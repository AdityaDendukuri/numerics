...
///
/// Usage:
///   gen_report <template.md> <out.md> <test.json> <bench.json>
///   <build_info.json> <plots_dir>
///
/// Reads template.md, substitutes every {{KEY}} with generated markdown, writes
/// out.md. Missing data (backend absent, JSON not found) leaves a *[not
...
/// Extract numeric value of key from a flat JSON object string (first
/// occurrence).
static double jnum(const std::string& obj, const std::string& key) {
  ...
  size_t end = np + 1;
  while (end < obj.size()
         && (isdigit(obj[end]) || obj[end] == '.' || obj[end] == 'e' || obj[end] == 'E'
             || obj[end] == '+' || obj[end] == '-'))
    ++end;
  try {
    return std::stod(obj.substr(np, end - np));
  } catch (...) {
    return 0.0;
  }
}
...
static std::vector<TestSuite> parse_gtest_json(const std::string& path) {
  ...
  for (auto& obj : split_objects(arr)) {
    ...
    if (!t.empty() && t.back() == 's')
      try {
        s.time_ms = std::stod(t.substr(0, t.size() - 1)) * 1000.0;
      } catch (...) {}
    if (!s.name.empty())
      result.push_back(s);
  }
...
static std::string backends_table(const BuildInfo& b) {
  std::string s = "| Backend | Status | Notes |\n"
                  "|---------|--------|-------|\n";
  auto row = [&](const char* name, bool found, const char* note) {
    s += std::string("| ") + name + " | " + (found ? "**found**" : "not found") + " | "
         + note + " |\n";
  };
  row("BLAS / cblas",
      b.has_blas,
...
static std::string tests_table(const std::vector<TestSuite>& suites,
  ...
  if (rows.empty())
    return "*No test data -- run `make report` to generate.*\n";

  std::string t = "| Suite | Tests | Passed | Failed | Time |\n"
                  "|-------|------:|-------:|-------:|-----:|\n";
  for (auto* s : rows) {
...
static long bench_size(const std::string& name) {
  ...
  if (pos == name.npos)
    return -1;
  try {
    return std::stol(name.substr(pos + 1));
  } catch (...) {
    return -1;
  }
}
...
static std::string bench_label(const std::string& name) {
  ...
  // FFT-specific backends first (more specific substrings before generic
  // ones)
  if (has("FFTBackend::stdsimd"))
    return "std::simd";
  if (has("FFTBackend::simd"))
    return "simd (AVX2/NEON)";
  if (has("FFTBackend::fftw"))
    return "fftw (FFTW3)";
...
/// @param use_bps  true = GB/s from bytes_per_second;  false = us from
/// real_time.
static std::string bench_table(const std::vector<BenchRun>& all,
  ...
  for (auto& r : all) {
    if (r.run_type != "iteration")
      continue;
    if (r.name.find("_BigO") != r.name.npos)
      continue;
    if (r.name.find("_RMS") != r.name.npos)
      continue;
    if (r.name.find(prefix) == r.name.npos)
      continue;
    long sz = bench_size(r.name);
    if (sz <= 0)
      continue;
    std::string lbl = bench_label(r.name);
    double val = use_bps ? r.bps / 1e9 : to_us(r.real_time, r.time_unit);
    if (data.find(lbl) == data.end())
      order.push_back(lbl);
    data[lbl][sz] = val;
    all_sizes.insert(sz);
  }
  ...
  if (data.empty())
    return "*No benchmark data -- run `make report` to generate.*\n";
  ...
  for (auto& lbl : order) {
    ...
    for (long n : sizes) {
      ...
      if (it != data[lbl].end()) {
        char buf[32];
        if (use_bps)
          snprintf(buf, sizeof(buf), " %.2f |", it->second);
        else
          snprintf(buf, sizeof(buf), " %.1f |", it->second);
        t += buf;
      } else {
        t += " -- |";
      }
...
static std::map<std::string, std::string> build_subs(const BuildInfo& info,
  ...
  s["BENCH_MATVEC_TABLE"] =
    bench_table(bench, "BM_Matvec", true, "*Throughput in GB/s. Higher is better.*");
  s["BENCH_DOT_TABLE"] =
    bench_table(bench, "BM_Dot", true, "*Throughput in GB/s. Higher is better.*");
  s["BENCH_AXPY_TABLE"] =
    bench_table(bench, "BM_Axpy", true, "*Throughput in GB/s. Higher is better.*");
  // Linalg
  s["BENCH_CG_TABLE"] =
    bench_table(bench, "BM_CG", false, "*Time in us. Lower is better.*");
  s["BENCH_THOMAS_TABLE"] = bench_table(bench,
                                        "BM_Thomas/",
                                        false,
                                        "*Time in us. Linear O(n) scaling expected.*");
  s["BENCH_BANDED_TABLE"] =
    bench_table(bench, "BM_Band", false, "*Time in us. Lower is better.*");
  // Factorization (3-way: seq vs omp vs LAPACK)
  s["BENCH_LU_TABLE"] =
    bench_table(bench, "BM_LU", false, "*GFLOP/s: 2/3 n^3 / time. Higher is better.*");
  s["BENCH_QR_TABLE"] =
    bench_table(bench, "BM_QR", false, "*GFLOP/s: 4/3 n^3 / time. Higher is better.*");
  // Eigensolvers (3-way: Jacobi seq vs omp vs LAPACK; plus Lanczos)
  s["BENCH_EIGSYM_TABLE"] =
    bench_table(bench, "BM_EigSym", false, "*Time in us. Lower is better.*");
  s["BENCH_LANCZOS_TABLE"] =
    bench_table(bench,
                "BM_Lanczos",
                false,
  ...
  s["BENCH_SVD_TABLE"] =
    bench_table(bench, "BM_SVD/", false, "*Time in us. Lower is better.*");
  s["BENCH_SVD_RAND_TABLE"] =
    bench_table(bench,
                "BM_SVD_Randomized",
                false,
  ...
  // Spectral
  s["BENCH_FFT_TABLE"] =
    bench_table(bench, "BM_FFT", true, "*Throughput in GB/s. Higher is better.*");
  s["BENCH_FFT_PLAN_TABLE"] =
    bench_table(bench,
                "BM_FFTPlan",
                true,
  ...
  while (i < tmpl.size()) {
    if (tmpl[i] == '{' && i + 1 < tmpl.size() && tmpl[i + 1] == '{') {
      ...
      if (end != tmpl.npos) {
        ...
        out += (it != subs.end()) ? it->second : "*[" + key + " -- data not available]*";
        i = end + 2;
        continue;
      }
    }
...
int main(int argc, char** argv) {
  ...
  if (dot != html_path.npos)
    html_path = html_path.substr(0, dot);
  html_path += ".html";
