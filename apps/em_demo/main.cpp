...
int main() {
    ...
    printf("Solving EM fields... "); fflush(stdout);
    ...
    num::viz::run("EM: Aluminum Rod + Magnet", WIN, WIN,
        [&](num::viz::Frame& f) {
            azimuth += 2.0f * 3.14159265f / (60.0f * 12.0f);  // full orbit in ~12 s
            ...
            for (int i = 0; i < NX; i += STRIDE) {
                ...
                float dbx, dby, dbz;
                dipole_B(i*DX, j*DX, k*DX, mag_x, mag_y, mag_z, dbx, dby, dbz);
                float mag = sqrtf((bx+dbx)*(bx+dbx) + (by+dby)*(by+dby) + (bz+dbz)*(bz+dbz));
            ...
            DrawCylinder({cast<float>(ROD_CX)*DX, 0.0f, cast<float>(ROD_CZ)*DX},
                         rod_wr, rod_wr, SIM_DOMAIN, 24, {180, 185, 195, 220});
            DrawCylinderWires({cast<float>(ROD_CX)*DX, 0.0f, cast<float>(ROD_CZ)*DX},
                              rod_wr, rod_wr, SIM_DOMAIN, 24, {100, 110, 130, 255});
            DrawCylinder({cast<float>(ROD_CX)*DX, SIM_DOMAIN - DX*0.5f, cast<float>(ROD_CZ)*DX},
                         rod_wr, rod_wr, DX*0.5f, 24, {220, 60, 60, 255});
            DrawCylinder({cast<float>(ROD_CX)*DX, 0.0f, cast<float>(ROD_CZ)*DX},
                         rod_wr, rod_wr, DX*0.5f, 24, {60, 80, 220, 255});

            // Magnet (N/S poles)
            f.sphere3d(mag_x, mag_y + MAG_VIS_R*0.5f, mag_z, MAG_VIS_R, {210, 60, 60, 255});
            f.sphere3d(mag_x, mag_y - MAG_VIS_R*0.5f, mag_z, MAG_VIS_R, { 60, 80, 210, 255});
            f.line3d(mag_x, mag_y - MAG_VIS_R*1.6f, mag_z,
                     mag_x, mag_y + MAG_VIS_R*1.6f, mag_z, num::viz::kWhite);
            ...
            for (int i = 0; i < NX; i += STRIDE) {
                ...
                f.line3d(wx, wy, wz, wx+bx/mag*len, wy+by/mag*len, wz+bz/mag*len, col);
                f.sphere3d(wx+bx/mag*len, wy+by/mag*len, wz+bz/mag*len, len*0.12f, col);
            }
            ...
            f.text("Aluminum Rod + Permanent Magnet", 16, 14, 16, num::viz::kWhite);
            f.textf(16, 36, 13, num::viz::kGray,
                    "solve %.0f ms | iters=%zu res=%.1e",
                    solve_ms, r_phi.iterations, r_phi.residual);
            f.textf(16, 52, 13, num::viz::kGray,
                    "B_max = %.3e T", cast<double>(B_max));
        },
        {15, 15, 20, 255});
}
