/// @file src/sim.cpp
/// @brief Implementation of TDSESolver -- 2-D TDSE via Strang splitting + Thomas algorithm.
///
...
///   5. kick_V(dt/2)    -- again
///
/// Eigenstates: num::lanczos on the real N^2xN^2 Hamiltonian (matrix-free matvec).
/// Bessel zeros: num::brent on J_m(x) for exact CircularWell eigenvalues.
...
TDSESolver::TDSESolver(int N_, double L_, double dt_)
    : N(N_), L(L_), h(L_ / static_cast<double>(N_ + 1)), dt(dt_),
...
void TDSESolver::set_potential(Potential p, double param) {
    ...
    if (p == Potential::Barrier) {
        ...
        for (int i = ix0; i <= std::min(ix1, N-1); ++i) {
            for (int j = 0; j < N; ++j) {
                double y = (j + 1.0) * h;
                bool in_slit1 = std::abs(y - (cy - gap_sep * 0.5)) < gap_w * 0.5;
                bool in_slit2 = std::abs(y - (cy + gap_sep * 0.5)) < gap_w * 0.5;
...
void TDSESolver::init_gaussian(double x0, double y0, double kx, double ky, double sigma) {
    for (int i = 0; i < N; ++i) {
        double x = (i + 1.0) * h;
        for (int j = 0; j < N; ++j) {
            double y = (j + 1.0) * h;
...
void TDSESolver::sweep_x(double tau) { adi_.sweep(psi, true,  tau); }
void TDSESolver::sweep_y(double tau) { adi_.sweep(psi, false, tau); }
...
double TDSESolver::compute_energy() const {
    ...
    // laplacian_stencil_2d gives lap[k] = (sum of 4 nbrs) - 4*psi[k] (Dirichlet)
...
void TDSESolver::compute_modes(int k) {
    // H is real symmetric -> eigenstates are real. Work entirely in real arithmetic.
    const int n2 = N * N;
    const double c = 1.0 / (2.0 * h * h);
    ...
    if (current_potential_ == Potential::CircularWell) {
        ...
        for (int m = 0; m <= 2 && mode_idx < got; ++m) {
            ...
            for (int nz = 1; nz <= 3 && mode_idx < got; ++nz) {
                auto Jm = [m](real x) { return bessel_j(m, x); };
                double lo = bracket_lo, hi = lo + 0.5;
                int scan = 0;
                while (Jm(lo) * Jm(hi) > 0 && scan < 200) { lo = hi; hi += 0.5; ++scan; }
                if (Jm(lo) * Jm(hi) > 0) break;
