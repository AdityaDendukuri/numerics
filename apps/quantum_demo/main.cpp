...
static Color gate_color(int kind, const std::string& label) {
    if (kind == 2) return {180,  60, 180, 255};  // SWAP: purple
    if (kind == 3) return { 40, 160, 160, 255};  // Toffoli/Fredkin: teal
    if (kind == 1) return { 40, 160, 160, 255};  // controlled: teal
    // Single-qubit by name
    if (label == "H")                             return {254, 188,  17, 255}; // gold
    if (label == "X" || label == "(+)")            return {210,  60,  60, 255}; // red
    if (label == "Y")                             return { 60, 180,  60, 255}; // green
    if (label == "Z")                             return { 60, 100, 210, 255}; // blue
    if (label == "S" || label == "Sdg")           return {200, 120,  40, 255}; // orange
    if (label == "T" || label == "Tdg")           return {200, 120,  40, 255}; // orange
    if (label[0] == 'R')                          return {140,  60, 200, 255}; // purple
    return {100, 140, 180, 255};                                               // default
}

// Preset circuits
struct Preset { std::string name; Circuit circuit; };
...
static std::vector<Preset> make_presets() {
    ...
    {
        Circuit c(2);
        c.h(0).h(1)                              // uniform superposition
    ...
    // 4. Quantum Teleportation  -- teleports RY(pi/3)|0> from q0 to q2
    {
        ...
        c.ry(pi / 3, 0)    // prepare state on q0
         .h(1).cx(1, 2)    // Bell pair between q1 and q2
         .cx(0, 1).h(0)    // Alice's encoding
...
static void draw_gate_box(int cx, int cy, int w, int h,
                          Color fill, const char* label, bool active) {
    int x = cx - w/2, y = cy - h/2;
    DrawRectangle(x, y, w, h, fill);
    if (active)
        DrawRectangleLinesEx({(float)x-1,(float)y-1,(float)(w+2),(float)(h+2)}, 2, COL_ACTIVE);
    else
        DrawRectangleLinesEx({(float)x,(float)y,(float)w,(float)h}, 1, {0,0,0,80});
...
static void draw_circuit(const std::vector<GateView>& views, int n_qubits,
                         int current_step, int active_gate,
                         int origin_x, int origin_y, int panel_w) {
    if (views.empty()) return;

    // Compute total columns
    int total_cols = 0;
    for (const auto& v : views) total_cols = std::max(total_cols, v.col + 1);

    // Wire y-positions
    auto wire_y = [&](int q) { return origin_y + 40 + q * WIRE_PITCH; };
    // Gate x-position
    auto gate_x = [&](int col) { return origin_x + 50 + col * GATE_W; };

    // Draw qubit labels and wires
    for (int q = 0; q < n_qubits; ++q) {
        int wy = wire_y(q);
        char label[16]; std::snprintf(label, sizeof(label), "q[%d]", q);
        DrawText(label, origin_x + 4, wy - 8, 16, COL_DIM);
        // Wire extends across all columns
        int wire_end_x = gate_x(total_cols);
        DrawLine(origin_x + 46, wy, std::min(wire_end_x + 10, origin_x + panel_w - 10), wy, COL_WIRE);
    }
    ...
    for (int i = 0; i < (int)views.size(); ++i) {
        ...
        if (v.kind == 0) {
            ...
            draw_gate_box(gx, c2, GATE_W - 8, GATE_H, base, "\xe2\x8a\x95", active);
            if (active) {
...
static void draw_histogram(const quantum::Statevector& sv, int n_qubits,
    ...
    for (int k = 0; k < N; ++k) {
        ...
        if (p < 0.001f) col = {40, 60, 80, 255};

        DrawRectangle(bx, by, bar_w, bar_h, col);
        DrawRectangleLinesEx({(float)bx,(float)(ay),(float)bar_w,(float)ah}, 1, {60,80,100,120});
        ...
        DrawText(("|" + lbl + "\xe2\x9f\xa9").c_str(), bx, ay + ah + 4, font, COL_DIM);

        // Probability text above bar
        if (p > 0.02f) {
            char pct[8]; std::snprintf(pct, sizeof(pct), "%3.0f%%", p*100.f);
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_R))    step = 0;
        if (IsKeyPressed(KEY_SPACE)) auto_step = !auto_step;

        for (int i = 0; i < 5; ++i)
            if (IsKeyPressed(KEY_ONE + i)) load_preset(i);

        if (auto_step) {
            auto_timer += GetFrameTime();
            if (auto_timer >= AUTO_INTERVAL) {
                auto_timer = 0.0;
                if (step < n_gates) ++step;
                else                auto_step = false;
            }
        }
        ...
        char step_buf[40];
        std::snprintf(step_buf, sizeof(step_buf), "STEP %d / %d", step, n_gates);
        int sw = MeasureText(step_buf, 18);
        DrawText(step_buf, WIN_W - sw - 12, 40, 18,
                 (step == n_gates) ? COL_GOLD : COL_DIM);

        // Auto indicator
        if (auto_step) DrawText("[AUTO]", WIN_W - 90, 12, 16, {100,210,100,255});

        // Circuit panel
        draw_panel(CIR_X, CIR_Y, CIR_W, CIR_H, "Circuit");
        draw_circuit(views, n_qubits, step, active,
                     CIR_X + 4, CIR_Y + 20, CIR_W - 8);

        // Histogram panel
        draw_panel(HIS_X, HIS_Y, HIS_W, HIS_H, "Measurement Probabilities");
        draw_histogram(sv, n_qubits, HIS_X + 4, HIS_Y + 20, HIS_W - 8, HIS_H - 30);

        // Stats strip (below circuit panel)
        int sy = CIR_Y + CIR_H + 10;
        DrawRectangle(CIR_X, sy, CIR_W, 80, COL_PANEL);
        DrawRectangleLinesEx({(float)CIR_X,(float)sy,(float)CIR_W,80}, 1, COL_WIRE);

        // Most likely state
        auto probs = quantum::probabilities(sv);
        int  best_k = (int)(std::max_element(probs.begin(), probs.end()) - probs.begin());
        ...
        std::snprintf(stat_buf, sizeof(stat_buf),
                      ...
                      best_lbl.c_str(), probs[best_k]*100.0);
        ...
        char ent_buf[48];
        std::snprintf(ent_buf, sizeof(ent_buf), "S(q[0]) = %.3f ebit%s",
                      S, S > 0.999 ? "  (max entangled)" : "");
        DrawText(ent_buf, CIR_X + 10, sy + 30, 16,
        ...
        DrawRectangleLinesEx({(float)HIS_X,(float)sy,(float)HIS_W,80}, 1, COL_WIRE);
        DrawText("All amplitudes", HIS_X + 10, sy + 8, 14, COL_DIM);

        int N = 1 << n_qubits;
        int row = 0;
        for (int k = 0; k < N && row < 3; ++k) {
            if (std::abs(sv[k]) < 0.005) continue;
            std::string lbl(n_qubits, '0');
            for (int b = 0; b < n_qubits; ++b)
                lbl[n_qubits-1-b] = ((k >> b) & 1) ? '1' : '0';
            char amp[64];
            std::snprintf(amp, sizeof(amp), "|%s\xe2\x9f\xa9 %+.3f%+.3fi",
                          lbl.c_str(), (float)sv[k].real(), (float)sv[k].imag());
        ...
        DrawText("[<-/->] Step   [SPACE] Auto   [R] Reset   [1-5] Load Circuit",
                 12, ctrl_y + 10, 16, COL_DIM);

        // Preset selector highlights
        for (int i = 0; i < 5; ++i) {
            char key[4]; std::snprintf(key, sizeof(key), "[%d]", i+1);
            int kx = WIN_W - 300 + i * 56;
            DrawText(key, kx, ctrl_y + 10, 16,
