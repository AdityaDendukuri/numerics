...
Gate gate_y() {
    return {cplx{0, 0}, cplx{0, -1}, cplx{0, 1}, cplx{0, 0}};
}
Gate gate_z() {
...
Gate gate_s() {
    return {cplx{1, 0}, cplx{0, 0}, cplx{0, 0}, cplx{0, 1}};
}
Gate gate_sdg() {
    return {cplx{1, 0}, cplx{0, 0}, cplx{0, 0}, cplx{0, -1}};
}
...
void apply_2q(Statevector& sv, int n_qubits, const Gate2Q& G, int q0, int q1) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        const cplx a00 = sv[i00], a01 = sv[i01], a10 = sv[i10], a11 = sv[i11];
...
std::array<cplx, 4> reduced_density_matrix(const Statevector& sv,
    ...
    for (int i = 0; i < N; ++i) {
        if (i & mask)
            continue;                       // visit each environment state once
