...
Circuit& Circuit::h(int q) {
    ops_.push_back({Op::H, q});
    return *this;
}
Circuit& Circuit::x(int q) {
    ops_.push_back({Op::X, q});
    return *this;
}
Circuit& Circuit::y(int q) {
    ops_.push_back({Op::Y, q});
    return *this;
}
Circuit& Circuit::z(int q) {
    ops_.push_back({Op::Z, q});
    return *this;
}
Circuit& Circuit::s(int q) {
    ops_.push_back({Op::S, q});
    return *this;
}
Circuit& Circuit::sdg(int q) {
    ops_.push_back({Op::Sdg, q});
    return *this;
}
Circuit& Circuit::t(int q) {
    ops_.push_back({Op::T, q});
    return *this;
}
Circuit& Circuit::tdg(int q) {
    ops_.push_back({Op::Tdg, q});
    return *this;
}
...
Circuit& Circuit::ccx(int c0, int c1, int tgt) {
    Op op{Op::CCX, c0, c1, tgt};
    ops_.push_back(op);
    return *this;
}
Circuit& Circuit::cswap(int ctrl, int q0, int q1) {
    Op op{Op::CSWAP, ctrl, q0, q1};
    ops_.push_back(op);
    return *this;
}
...
Circuit& Circuit::gate(const quantum::Gate2Q& G,
                       int                    q0,
                       int                    q1,
                       std::string            label) {
...
void Circuit::apply_op(const Op& op, quantum::Statevector& sv) const {
    ...
    switch (op.type) {
        case Op::H:
            apply_1q(sv, n_qubits_, gate_h(), op.q0);
            break;
        case Op::X:
            apply_1q(sv, n_qubits_, gate_x(), op.q0);
            break;
        case Op::Y:
            apply_1q(sv, n_qubits_, gate_y(), op.q0);
            break;
        case Op::Z:
            apply_1q(sv, n_qubits_, gate_z(), op.q0);
            break;
        case Op::S:
            apply_1q(sv, n_qubits_, gate_s(), op.q0);
            break;
        case Op::Sdg:
            apply_1q(sv, n_qubits_, gate_sdg(), op.q0);
            break;
        case Op::T:
            apply_1q(sv, n_qubits_, gate_t(), op.q0);
            break;
        case Op::Tdg:
            apply_1q(sv, n_qubits_, gate_tdg(), op.q0);
            break;
        case Op::RX:
            apply_1q(sv, n_qubits_, gate_rx(op.theta), op.q0);
            break;
        case Op::RY:
            apply_1q(sv, n_qubits_, gate_ry(op.theta), op.q0);
            break;
        case Op::RZ:
            apply_1q(sv, n_qubits_, gate_rz(op.theta), op.q0);
            break;
        case Op::P:
            apply_1q(sv, n_qubits_, gate_p(op.lambda), op.q0);
            break;
        case Op::U:
            apply_1q(sv, n_qubits_, gate_u(op.theta, op.phi, op.lambda), op.q0);
            break;

        case Op::CX:
            apply_cnot(sv, n_qubits_, op.q0, op.q1);
            break;
        case Op::CY:
            apply_cy(sv, n_qubits_, op.q0, op.q1);
            break;
        case Op::CZ:
            apply_cz(sv, n_qubits_, op.q0, op.q1);
            break;
        case Op::CP:
            apply_cp(sv, n_qubits_, op.lambda, op.q0, op.q1);
            break;
        case Op::SWAP:
            apply_swap(sv, n_qubits_, op.q0, op.q1);
            break;

        case Op::CCX:
            apply_toffoli(sv, n_qubits_, op.q0, op.q1, op.q2);
            break;
        case Op::CSWAP:
            apply_cswap(sv, n_qubits_, op.q0, op.q1, op.q2);
            break;

        case Op::Custom1Q:
            apply_1q(sv, n_qubits_, op.g1, op.q0);
            break;
        case Op::Custom2Q:
            apply_2q(sv, n_qubits_, op.g2, op.q0, op.q1);
            break;
    }
}
...
std::pair<std::vector<int>, int> Circuit::compute_layout() const {
    ...
    for (int i = 0; i < (int)ops_.size(); ++i) {
        ...
        int         c  = last[op.q0];
        if (op.q1 >= 0)
            c = std::max(c, last[op.q1]);
        if (op.q2 >= 0)
            c = std::max(c, last[op.q2]);
        gate_col[i] = c + 1;
        total       = std::max(total, gate_col[i] + 1);
        last[op.q0] = gate_col[i];
        if (op.q1 >= 0)
            last[op.q1] = gate_col[i];
        if (op.q2 >= 0)
            last[op.q2] = gate_col[i];
    }
    ...
    for (int i = 0; i < (int)ops_.size(); ++i) {
        ...
        switch (op.type) {
            ...
            case Op::Tdg:
                ...
                break;
            case Op::RX:
                v.label = "Rx(" + fmt_angle(op.theta) + ")";
                v.kind  = 0;
                break;
            case Op::RY:
                v.label = "Ry(" + fmt_angle(op.theta) + ")";
                v.kind  = 0;
                break;
            case Op::RZ:
                v.label = "Rz(" + fmt_angle(op.theta) + ")";
                v.kind  = 0;
                break;
            case Op::P:
                v.label = "P(" + fmt_angle(op.lambda) + ")";
                v.kind  = 0;
                break;
            case Op::U:
                v.label = "U";
            ...
            case Op::CZ:
                ...
                break;
            case Op::CP:
                v.label = "CP(" + fmt_angle(op.lambda) + ")";
                v.kind  = 1;
                break;
            case Op::SWAP:
                v.label = "SWAP";
...
std::string Circuit::diagram() const {
    ...
    // Grid: 2*n_qubits-1 display rows (qubit rows interleaved with connector
    // rows) Each cell is a fixed 3-char string
