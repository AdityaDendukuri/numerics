...
///   sweep_umbrella()   -- umbrella sampling constrained to a nucleus-size
///   window
///
...
struct IsingLattice {
    ...
    IsingLattice()
        : spins(NN, 1.0)
        , beta(1.0
               / (1.0 / 0.58)) // T ~ 1.724  (matches IsingNucleation defaults)
        , F(0.0)
        , ones_(NN, 1.0)
        , nbr_buf_(NN, 0.0)
        , adj_(build_adj())
    ...
    /// Square spin-down nucleus of area ~r centred at the middle of the
    /// lattice.
    ...
    int sweep_umbrella(int lo, int hi, num::ClusterResult& det) {
        auto result = num::markov::umbrella_sweep_prob(
            ...
            num::markov::UmbrellaWindow{static_cast<idx>(lo),
                                        static_cast<idx>(hi)},
            rng_);
    ...
    real magnetization() const {
        return std::abs(dot(spins, ones_, num::Backend::seq))
               / static_cast<real>(NN);
    }
    ...
    real energy_per_spin() {
        ...
        real fld  = -F * dot(spins, ones_, num::Backend::seq)
                   / static_cast<real>(NN);
        return coup + fld;
    }

    /// Mean-field order parameter: solve m = tanh(4*beta*J*m + beta*F) via
    /// num::newton().
  ...
  private:
    ...
    // Boltzmann acceptance table: boltz_[spin_idx][nbr_idx] = min(1,
    // exp(-beta*dE)) spin_idx: 0 = s=-1, 1 = s=+1 nbr_idx:  (nbr_sum/2 + 2) in
    // {0..4}
    void rebuild_boltz() {
    ...
    static int spin_idx(real s) {
        return s < 0.0 ? 0 : 1;
    }
    static int nbr_idx(real ns) {
        return static_cast<int>(ns * 0.5 + 2.0);
    }
