...
// Color mapped from body mass: blue-white (low) to yellow (mid) to orange-red (high)
...
float disp_r(double m) { return DISP_SCALE * std::cbrt(float(m)); }
float phys_r(double m) { return PHYS_SCALE * std::cbrt(float(m)); }
...
double NBodySim::kinetic_energy() const {
    ...
    double KE = 0.0;
    for (int i = 0; i < n(); ++i)
        KE += 0.5 * bodies[i].mass * (vel.x(i)*vel.x(i) + vel.y(i)*vel.y(i));
    return KE;
}
...
double NBodySim::potential_energy() const {
    ...
    for (int i = 0; i < n(); ++i) {
        for (int j = i+1; j < n(); ++j) {
            ...
            PE -= G * bodies[i].mass * bodies[j].mass / std::sqrt(dx*dx + dy*dy + soft*soft);
        }
    }
    return PE;
}
    ...
    while (i < n() && n() > 1) {
        ...
        for (int j = i + 1; j < n(); ++j) {
            ...
            if (j != last) {
                ...
                pos.x(j) = pos.x(last);  pos.y(j) = pos.y(last);
                vel.x(j) = vel.x(last);  vel.y(j) = vel.y(last);
            }
            bodies.pop_back();
...
void NBodySim::reset(Scenario s) {
    ...
    if (s == Scenario::Figure8) {
        ...
        for (int i = 0; i < N_gal; ++i) {
            ...
            // Tangential direction (counterclockwise): (-sin(theta), cos(theta))
