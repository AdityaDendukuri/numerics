...
///     query  : O(9 * avg_bucket)  -- pointer-chase linked list in each of 9
///     cells
///
...
///     pairs  : iterate_pairs()  -- visits each (i,j) pair once (Newton 3rd
///     law)
///
...
class SpatialHash {
  public:
    ...
    /// Point query: calls f(int j) for every candidate in the 3x3
    /// neighbourhood. Caller must verify |r_ij| < cutoff.  (Same contract as
    /// before.)
    template<typename F>
