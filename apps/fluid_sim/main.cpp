...
static physics::FluidParams make_params() {
    ...
    p.gy          = -9.81f;
    p.dt          = 0.001f;
    p.xmin        = 0.0f;  p.xmax = DOM_W;
    p.ymin        = 0.0f;  p.ymax = DOM_H;
    p.restitution = 0.01f;
    p.alpha_T     = 0.005f;
    p.h_conv      = 8.0f;
    return p;
...
static void setup(physics::FluidSolver& solver) {
    ...
    hot.x = 0.74f; hot.y = 0.22f; hot.radius = 0.10f;
    hot.temperature = 90.0f; hot.mass = 5000.0f;
    hot.fixed = true; hot.is_heat_source = true;
    solver.add_body(hot);

    physics::RigidBody cold{};
    cold.x = 0.78f; cold.y = 0.57f; cold.radius = 0.08f;
    cold.temperature = 0.0f; cold.mass = 5000.0f;
    cold.fixed = true; cold.is_heat_source = true;
...
int main() {
    ...
    num::viz::run("2D SPH Fluid: Dam Break", WIN, cast<int>(WIN * DOM_H / DOM_W),
        [&](num::viz::Frame& f) {
            if (f.reset_pressed()) setup(solver);
            ...
            // Domain boundary
            f.rect_outline(0, 0, cast<float>(f.width), cast<float>(f.height),
                           {60, 60, 80, 255});
            ...
            for (const auto& body : solver.bodies()) {
                ...
                auto fc = num::viz::heat_color((body.temperature - T_min) / T_rng);
                fc.a = 180;
                f.dot(px, py, fc, r);
                f.circle(px, py, r, {220, 220, 220, 200});
                const char* label = body.temperature > 50.0f ? "HOT" : "COLD";
                f.text(label, px - 14, py - 8, 14,
                       body.temperature > 50.0f
                           ? num::viz::Color{255, 80, 80}
                           : num::viz::kSkyBlue);
            }
            ...
            for (const auto& p : solver.particles()) {
                ...
                f.dot(px, py, num::viz::heat_color((p.temperature - T_min) / T_rng), PRAD);
            }
