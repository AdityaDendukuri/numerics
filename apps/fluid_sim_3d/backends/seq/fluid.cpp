...
void compute_density_pressure(std::vector<Particle3D>& particles,
    ...
    for (Particle3D& p : particles) p.density = m * W0;
...
void compute_forces(std::vector<Particle3D>& particles,
    ...
    for (Particle3D& p : particles) { p.ax = params.gx; p.ay = params.gy; p.az = params.gz; }
    ...
    grid.iterate_pairs([&](int i, int j) {
        ...
        const float fpx = -m * pterm * gx, fpy = -m * pterm * gy, fpz = -m * pterm * gz;
        pi.ax += fpx;  pi.ay += fpy;  pi.az += fpz;
        pj.ax -= fpx;  pj.ay -= fpy;  pj.az -= fpz;
        ...
        pi.ax += fvx;  pi.ay += fvy;  pi.az += fvz;
        pj.ax -= fvx;  pj.ay -= fvy;  pj.az -= fvz;
    });
}
...
void integrate_bodies(std::vector<RigidBody3D>& bodies, const FluidParams3D& params) {
    ...
    for (RigidBody3D& b : bodies) {
        if (b.fixed) continue;
        b.vx += params.gx * dt;  b.vy += params.gy * dt;  b.vz += params.gz * dt;
        ...
        if (b.z - r < params.zmin) { b.z = params.zmin + r; b.vz =  std::abs(b.vz) * e; }
        if (b.z + r > params.zmax) { b.z = params.zmax - r; b.vz = -std::abs(b.vz) * e; }
    }
}
