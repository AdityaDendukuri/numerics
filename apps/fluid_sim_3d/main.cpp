...
struct Hose {
    ...
    static Vector3 norm(Vector3 v) {
        ...
        return len < 1e-8f ? Vector3{0, 1, 0}
    ...
    void emit(physics::FluidSolver3D& solver,
              int                     count,
              int                     max_n = 1800) const {
        if (cast<int>(solver.particles().size()) >= max_n)
            return;
        const float max_r = tanf(cone_half);
        for (int i = 0; i < count; ++i) {
            float angle = cast<float>(GetRandomValue(0, 10000)) / 10000.0f
                          * 2.0f * kPI;
            ...
            solver.add_particle(source.x + jx,
                                source.y + jy,
                                source.z + jz,
                                d.x * speed,
                                d.y * speed,
...
int main() {
    ...
    Hose cold, hot;
    cold.init({0.77f, 0.76f, 0.02f},
              {0.4f, 0.4f, 0.4f},
              5.0f,
              1.8f,
              kPI / 90.0f);
    hot.init({0.03f, 0.76f, 0.02f},
             {0.4f, 0.4f, 0.4f},
             80.0f,
             1.8f,
             kPI / 90.0f);

    num::viz::run("3D SPH Fluid: Opposing Hoses",
                  WIN,
                  WIN,
                  [&](num::viz::Frame& f) {
                      if (f.reset_pressed()) {
                      ...
                      // Nozzles
                      f.sphere3d(cold.source.x,
                                 cold.source.y,
                                 cold.source.z,
                                 0.022f,
                                 num::viz::kSkyBlue);
                      f.sphere3d(hot.source.x,
                                 hot.source.y,
                                 hot.source.z,
                                 0.022f,
                                 {255, 140, 20, 255});
                      ...
                      for (const auto& p : solver.particles()) {
                          auto c = num::viz::heat_color((p.temperature - T_min)
                                                        / T_rng);
