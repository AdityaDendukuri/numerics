...
///
/// Sliders: temperature T, external field h, nucleus target size, window half-width.
/// Spin up = light gray, nucleus cluster = red, spin down = dark.
...
int main() {
    ...
    num::viz::run("Ising: Nucleation (Umbrella Sampling)", WIN, WIN,
        [&](num::viz::Frame& f) {
            ...
            f.field(N, [&](int col, int row) -> num::viz::Color {
                int idx = row * N + col;
                if (lat.spins[idx] > 0.0)            return {240, 240, 240, 255};
                if (det.id[idx] == det.largest_id)   return {220,  50,  50, 255};
                return {15, 15, 15, 255};
            });

            // Nucleus size overlay
            f.textf(WIN - 190, 8, 14, num::viz::kWhite,
                    "nucleus: %d spins", det.largest_size);
        });
}
