...
class BasicEulerSteps {
  ...
  public:
    explicit BasicEulerSteps(RHS f, State y0, ODEParams p = {})
        : f_(std::move(f)), y_(std::move(y0)), dydt_(y_.size()), t_(p.t0), t1_(p.tf), h_(p.h) {}

    struct iterator {
        BasicEulerSteps *owner_;
...
using EulerSteps = BasicEulerSteps<ODERhsFn, Vector>;
...
class BasicRK45Steps {
    ...
    void advance() {
        ...
        for (;;) {
            ...
            for (idx i = 0; i < n; ++i) {
                ytmp_[i] = y_[i] + (h_ * ((detail::rk45_a31 * k1_[i]) + (detail::rk45_a32 * k2_[i])));
            }
            f_(t_ + (3 * h_ / 10.0), ytmp_, k3_);

            for (idx i = 0; i < n; ++i) {
                ytmp_[i] =
                    y_[i] + (h_ * ((detail::rk45_a41 * k1_[i]) + (detail::rk45_a42 * k2_[i]) + (detail::rk45_a43 * k3_[i])));
            }
            f_(t_ + (4 * h_ / 5.0), ytmp_, k4_);

            for (idx i = 0; i < n; ++i) {
                ytmp_[i] = y_[i] + (h_ * ((detail::rk45_a51 * k1_[i]) + (detail::rk45_a52 * k2_[i]) +
            ...
            for (idx i = 0; i < n; ++i) {
                ytmp_[i] =
                    y_[i] + (h_ * ((detail::rk45_a61 * k1_[i]) + (detail::rk45_a62 * k2_[i]) + (detail::rk45_a63 * k3_[i]) +
                                   (detail::rk45_a64 * k4_[i]) + (detail::rk45_a65 * k5_[i])));
            }
            f_(t_ + h_, ytmp_, k6_);

            for (idx i = 0; i < n; ++i) {
                ytmp_[i] = y_[i] + (h_ * ((detail::rk45_b1 * k1_[i]) + (detail::rk45_b3 * k3_[i]) + (detail::rk45_b4 * k4_[i]) +
                                          (detail::rk45_b5 * k5_[i]) + (detail::rk45_b6 * k6_[i])));
            }
            f_(t_ + h_, ytmp_, k7_);

            for (idx i = 0; i < n; ++i) {
                err_[i] = h_ * ((k1_[i] * detail::rk45_e1) + (detail::rk45_e3 * k3_[i]) + (detail::rk45_e4 * k4_[i]) +
                                (k5_[i] * detail::rk45_e5) + (detail::rk45_e6 * k6_[i]) + (detail::rk45_e7 * k7_[i]));
            }
  ...
  public:
    explicit BasicRK45Steps(RHS f, State y0, ODEParams p = {})
        : f_(std::move(f)), y_(std::move(y0)), k1_(y_.size()), k2_(y_.size()), k3_(y_.size()),
...
class BasicVerletSteps {
  ...
  public:
    explicit BasicVerletSteps(Accel accel, State q0, State v0, ODEParams p = {})
...
using VerletSteps = BasicVerletSteps<AccelFn, Vector>;

/// Lazy fourth-order Yoshida symplectic trajectory for q''=a(q).
template <typename Accel = AccelFn, typename State = Vector>
class BasicYoshida4Steps {
    Accel accel_{};
    State q_{}, v_{}, acc_{};
    real t_ = 0.0, t1_ = 0.0, h_ = 0.0;
    idx steps_ = 0;
  ...
  public:
    explicit BasicYoshida4Steps(Accel accel, State q0, State v0, ODEParams p = {})
        : accel_(std::move(accel)), q_(std::move(q0)), v_(std::move(v0)), acc_(q_.size()),
          t_(p.t0), t1_(p.tf), h_(p.h) {}

    struct iterator {
        BasicYoshida4Steps *owner_;
        SymplecticStep operator*() const { return {owner_->t_, owner_->q_, owner_->v_}; }
        iterator &operator++() {
            owner_->advance();
            return *this;
...
using Yoshida4Steps = BasicYoshida4Steps<AccelFn, Vector>;

/// Lazy non-symplectic fourth-order trajectory for q''=a(q).
template <typename Accel = AccelFn, typename State = Vector>
class BasicRK4_2ndSteps {
    Accel accel_{};
  ...
  public:
    explicit BasicRK4_2ndSteps(Accel accel, State q0, State v0, ODEParams p = {})
...
using RK4_2ndSteps = BasicRK4_2ndSteps<AccelFn, Vector>;
