...
class StructuredMatrix final : public props_detail::square_tag<Ax> {
  public:
    ...
    using math_propositions = std::conditional_t<
    ...
    void apply(const X &x, Y &y) const {
        if constexpr (requires(const Mat &m, const X &in, Y &out) { m.apply(in, out); }) {
            ...
            props_detail::MatrixAsOperator<Mat>{A_}.apply(x, y);
        }
    }
...
/// `preset::production` the checks compile away.
///
/// @tparam Ax Axiomatic property tag (e.g. `property::spd`, `property::self_adjoint`).
/// @tparam Mat Concrete matrix container type.
/// @param A Input matrix to wrap.
/// @param loc Call site location for diagnostics.
/// @return `StructuredMatrix<Mat, Ax>` carrying verified property evidence.
...
///
/// Wraps matrix in `SquareMatrix<Mat>`, satisfying `SquareMatrixLike` concepts
/// required by direct solvers like `lu()`.
///
/// @tparam Mat Concrete matrix container type.
/// @param A Matrix whose squareness is asserted.
/// @return `SquareMatrix<Mat>` wrapper carrying square shape evidence.
/// @see make_square
template <class Mat = Matrix>
...
/// @return `SymmetricMatrix<Mat>` wrapper accepted by eigensolvers and MINRES.
/// @see make_symmetric
template <class Mat = Matrix>
...
/// @return `PSDMatrix<Mat>` wrapper carrying PSD evidence.
template <class Mat = Matrix>
...
/// @param loc Call site location for diagnostics.
/// @return `SPDMatrix<Mat>` wrapper carrying SPD evidence.
/// @see make_spd
template <class Mat = Matrix>
...
/// @param loc Source location for diagnostics.
/// @return `HermitianMatrix<Mat>` wrapper carrying Hermitian evidence.
template <class Mat = Matrix>
...
/// @return `SquareMatrix<Mat>` wrapper carrying verified square dimension evidence.
template <class Mat = Matrix>
...
/// @return `SymmetricMatrix<Mat>` wrapper carrying verified symmetry evidence.
template <class Mat = Matrix>
[[nodiscard]] inline SymmetricMatrix<Mat>
...
/// @return `SPDMatrix<Mat>` wrapper carrying verified SPD evidence.
template <class Mat = Matrix>
[[nodiscard]] inline SPDMatrix<Mat>
