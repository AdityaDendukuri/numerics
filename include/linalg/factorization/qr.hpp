...
/// ||A*x - b||_2 and is obtained by back-substituting into R[:n,:n] * x =
/// (Q^T*b)[:n]. The residual norm is ||(Q^T*b)[n:]||_2.
struct QRResult {
  Matrix Q; ///< mxm orthogonal
  Matrix R; ///< mxn upper triangular
};

/// @brief QR factorization of an mxn matrix A (m >= n) via Householder
/// reflections.
///
/// Each Householder step k:
///   1. Extract column k of the current working matrix from row k downward.
///   2. Compute v = x + sign(x[0]) * ||x|| * e_1  (sign chosen to avoid
///   cancellation).
///   3. Normalise v.
...
///
/// @param backend  Backend::lapack uses LAPACKE_dgeqrf (default when
/// available).
