...
SolverResult cg(const Matrix &A, const Vector &b, Vector &x, real tol = 1e-10, idx max_iter = 1000,
                Backend backend = default_backend);

/// Solve a dense system whose SPD property is represented in its type.
inline SolverResult cg(const linalg::SPDMatrix<Matrix> &A, const Vector &b, Vector &x,
                       real tol = 1e-10, idx max_iter = 1000, Backend backend = default_backend) {
...
template <class Op>
requires LinearOperator<Op, Vector, Vector> SolverResult
cg_operator_impl(const Op &A, const Vector &b, Vector &x, real tol, idx max_iter, Backend backend) {
    const idx n = b.size();
...
requires SPDLinearOperator<Op, Vector, Vector>
    SolverResult cg(const Op &A, const Vector &b, Vector &x, real tol = 1e-10, idx max_iter = 1000,
                    Backend backend = default_backend) {
