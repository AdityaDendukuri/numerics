...
/// @brief Phase/probability colormap for quantum wavefunctions.
...
struct Frame {
    ...
    void dot(float x, float y, Color c, float r = 3.0f) { DrawCircleV({x, y}, r, to_rl(c)); }
    ...
    // Pixel field covering the full window.
    template <class Fn>
    void field(int N, Fn color_fn) {
        ...
        DrawTexturePro(cv.tex, {0, 0, float(N), float(N)}, {0, 0, float(width), float(height)},
                       {0, 0}, 0.0f, ::WHITE);
    }
