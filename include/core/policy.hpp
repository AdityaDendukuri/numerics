...
/// This enum covers linalg (vector, matrix, solvers, factorization, eigen, svd).
/// Other modules define their own -- e.g. spectral/ uses FFTBackend.
///
///   num::seq      -- naive serial C++              (always available)
///   num::blocked  -- cache-blocked, no intrinsics  (compiler auto-vectorizes)
///   num::simd     -- hand-written SIMD             (AVX2 on x86, NEON on AArch64)
///   num::blas     -- cblas / LAPACKE               (optional, NUMERICS_HAS_BLAS)
///   num::omp      -- OpenMP parallel               (optional, NUMERICS_HAS_OMP)
///   num::gpu      -- CUDA                          (optional, NUMERICS_HAS_CUDA)
#pragma once
...
//              machine has multiple cores; thread overhead amortises for n > ~256.
...
/// Default backend for dense vector/matrix ops (matmul, matvec, dot, axpy, etc.).
/// Automatically selected at configure time: blas > omp > simd > blocked.
...
//
// Rationale:
//   lapack  -- LAPACKE (dgetrf, dgeqrf, dgesdd, dsyevd, etc.): decades-optimised,
//              BLAS-3 internally, fastest for n > ~64.
//   omp     -- our parallel Jacobi eigensolver; meaningful when LAPACK is absent.
//   seq     -- our textbook implementations (Doolittle LU, Householder QR, etc.).
//
