...
inline void matvec(const Matrix &A, const Vector &x, Vector &y, Tag = {}) {
    if constexpr (std::is_same_v<Tag, backend::blas_t> || std::is_same_v<Tag, backend::lapack_t>) {
        backends::blas::matvec(A, x, y);
    } else if constexpr (std::is_same_v<Tag, backend::omp_t>) {
        backends::omp::matvec(A, x, y);
    } else if constexpr (std::is_same_v<Tag, backend::simd_t>) {
        backends::simd::matvec(A, x, y);
    } else if constexpr (std::is_same_v<Tag, backend::gpu_t>) {
...
template <class Tag = backend::default_t>
inline void matmul(const Matrix &A, const Matrix &B, Matrix &C, Tag = {}) {
    if constexpr (std::is_same_v<Tag, backend::blas_t> || std::is_same_v<Tag, backend::lapack_t>) {
    ...
    } else if constexpr (std::is_same_v<Tag, backend::blocked_t>) {
...
// -----------------------------------------------------------------------------
// BasicMatrix::apply implementation
// -----------------------------------------------------------------------------

template <std::floating_point T>
template <class X, class Y>
inline void BasicMatrix<T>::apply(const X &x, Y &y) const {
    if constexpr (std::is_same_v<T, real> && std::is_same_v<X, Vector> && std::is_same_v<Y, Vector>) {
        matvec(*this, x, y);
    } else {
