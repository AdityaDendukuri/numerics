...
class BasicVector {
  public:
    ...
    explicit BasicVector(idx n) : n_(n), data_(n > 0 ? new T[n]() : nullptr) {}

    /// Construct n elements initialized to val.
    BasicVector(idx n, T val) : n_(n), data_(n > 0 ? new T[n] : nullptr) {
        if (n_ > 0) std::fill_n(data_.get(), n_, val);
    }

    /// Copy values from an initializer list.
    BasicVector(std::initializer_list<T> init)
        : n_(init.size()), data_(init.size() > 0 ? new T[init.size()] : nullptr) {
    ...
    explicit BasicVector(std::span<const T> values)
        : n_(values.size()), data_(values.size() > 0 ? new T[values.size()] : nullptr) {
    ...
    BasicVector(const BasicVector &o) : n_(o.n_), data_(o.n_ > 0 && o.data_ ? new T[o.n_] : nullptr) {
        if (n_ > 0 && o.data_) {
