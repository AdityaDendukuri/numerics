/// @file pde/fields.hpp
/// @brief 3D scalar and vector fields on Cartesian grids, with PDE field solvers.
///
/// ScalarField3D  -- potential phi(x,y,z) on a uniform grid with trilinear sampling.
/// VectorField3D  -- three ScalarField3D components; sample() returns trilinear (fx,fy,fz).
/// FieldSolver    -- static PDE utilities (Poisson solve, gradient, divergence, curl).
/// MagneticSolver -- static utilities for J = -sigma*grad(phi) and B = curl(A) via vector Poisson.
#pragma once
...
public:
    ...
    void set(int i, int j, int k, double v) { grid_.set(i,j,k,v); }
    void fill(double v)                      { grid_.fill(v); }
...
public:
    ...
    /// Internally solves the SPD system (-Laplacian)phi = -source via matrix-free CG.
    ...
    ///
    /// Typical use: current flow in a heterogeneous conductor (coeff = conductivity sigma).
    /// Imposes BCs via symmetric penalty elimination so the system remains SPD -> CG converges.
    /// Neumann (zero normal flux) on all non-BC boundaries.
...
public:
    ...
    /// Solves Laplacian(A) = -mu0*J (Coulomb gauge, Dirichlet A=0) via three CG solves,
    /// then returns B = curl(A).
