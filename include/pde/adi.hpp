/// @file pde/adi.hpp
/// @brief Crank-Nicolson ADI solver for 2D parabolic equations via fiber sweeps.
///
...
///   d(u)/dt     = kappa*Lap(u)                     (complex-coefficient diffusion)
///
...
/// covering the full Strang splitting: sweep_x(dt/2) -> sweep_y(dt) -> sweep_x(dt/2).
///
...
struct CrankNicolsonADI {
    ...
    /// Selects the half-step tridiagonal for tau < 0.75*dt, full-step otherwise.
    void sweep(CVector& psi, bool x_axis, double tau) const {
        ...
        if (x_axis) col_fiber_sweep(psi, N, apply);
        else        row_fiber_sweep(psi, N, apply);
    }
