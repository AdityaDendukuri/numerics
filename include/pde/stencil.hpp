...
inline real sample_2d_periodic(const ScalarField2D& g,
                               real px,
                               real py,
                               real ox = 0.0,
                               real oy = 0.0) {
...
inline void gradient_3d(const ScalarField3D& phi,
                        ScalarField3D& gx,
                        ScalarField3D& gy,
                        ScalarField3D& gz) {
...
inline void divergence_3d(const ScalarField3D& fx,
                          const ScalarField3D& fy,
                          const ScalarField3D& fz,
                          ScalarField3D& out) {
...
inline void curl_3d(const ScalarField3D& ax,
                    const ScalarField3D& ay,
                    const ScalarField3D& az,
                    ScalarField3D& bx,
                    ScalarField3D& by,
                    ScalarField3D& bz) {
  ...
  for (int k = 0; k < nz; ++k) {
    for (int j = 0; j < ny; ++j) {
      for (int i = 0; i < nx; ++i) {
        ...
        int kp = std::min(k + 1, nz - 1), km = std::max(k - 1, 0);
        bx(i, j, k) =
          (az(i, jp, k) - az(i, jm, k) - ay(i, j, kp) + ay(i, j, km)) * inv2dx;
        by(i, j, k) =
          (ax(i, j, kp) - ax(i, j, km) - az(ip, j, k) + az(im, j, k)) * inv2dx;
        bz(i, j, k) =
