...
///   sample_2d_periodic             -- bilinear interpolation with stagger offset
...
/// Compute y[i,j] = x[i+1,j] + x[i-1,j] + x[i,j+1] + x[i,j-1] - 4*x[i,j]/// Dirichlet: out-of-bounds neighbors contribute 0.
...
///
/// @param ox  x-axis origin offset in physical units (0 for unstaggered, h/2 for v-face)
/// @param oy  y-axis origin offset in physical units (0 for unstaggered, h/2 for u-face)
///
/// MAC grid usage:
/// \f[
...
/// \f]
inline real sample_2d_periodic(const Vector& field, idx N, real h,
                                real px, real py, real ox, real oy) {
    real fx = std::fmod((px - ox) / h, static_cast<real>(N));
    real fy = std::fmod((py - oy) / h, static_cast<real>(N));
    if (fx < 0.0) fx += N;
    if (fy < 0.0) fy += N;
    idx  i0 = static_cast<idx>(fx) % N;   idx i1 = (i0 + 1) % N;
    real fi  = fx - std::floor(fx);
    idx  j0 = static_cast<idx>(fy) % N;   idx j1 = (j0 + 1) % N;
...
/// Compute the negative Laplacian: y = -Lap(x) = (6x[i,j,k] - sum of 6 nbrs) / dx^2
...
/// Compute the central-difference gradient of a scalar field./// One-sided differences at domain boundaries.
