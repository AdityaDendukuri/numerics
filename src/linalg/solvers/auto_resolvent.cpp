...
AutoResolventSolver::AutoResolventSolver(const SparseMatrix& matrix,
  ...
  impl_->sparse = std::make_unique<SparseResolventSolver>(
    matrix,
    SparseResolventOptions{.symmetric_pattern = options.symmetric_pattern});
}
...
AutoResolventSolver& AutoResolventSolver::operator=(
  AutoResolventSolver&&) noexcept = default;
...
void AutoResolventSolver::solve(const std::vector<cplx>& rhs,
