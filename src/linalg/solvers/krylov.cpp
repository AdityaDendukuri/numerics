...
SolverResult gmres(MatVecFn matvec_fn, idx n, const Vector& b, Vector& x,
    ...
    while (total_iters < max_iter) {
        ...
        real beta = 0.0;
        for (idx i = 0; i < n; ++i) beta += r[i] * r[i];
        beta = std::sqrt(beta);
        ...
        for (; j < restart && total_iters < max_iter; ++j, ++total_iters) {
            ...
            for (idx i = 0; i <= j; ++i) {
                real h = 0.0;
                for (idx k = 0; k < n; ++k) h += w[k] * V[i][k];
                H[j][i] = h;
                for (idx k = 0; k < n; ++k) w[k] -= h * V[i][k];
            }
            real h_next = 0.0;
            ...
            if (denom < 1e-15) { cs[j] = 1.0; sn[j] = 0.0; }
            else               { cs[j] = h0 / denom; sn[j] = h1 / denom; }
            ...
            if (result.residual < tol) { result.converged = true; ++j; break; }
        }
        ...
        for (idx i = m; i > 0; ) {
            --i;
            y[i] = g[i];
            for (idx k = i + 1; k < m; ++k) y[i] -= H[k][i] * y[k];
            y[i] /= H[i][i];
        }

        for (idx i = 0; i < m; ++i)
            for (idx k = 0; k < n; ++k) x[k] += y[i] * V[i][k];
...
// Dense overload  -- wraps the matrix-free core with a backend-parameterized matvec
SolverResult gmres(const Matrix& A, const Vector& b, Vector& x,
                   real tol, idx max_iter, idx restart, Backend backend) {
    if (A.rows() != A.cols())
        throw std::invalid_argument("GMRES requires a square matrix");
    idx n = A.rows();
    MatVecFn mv = [&](const Vector& in, Vector& out) {
