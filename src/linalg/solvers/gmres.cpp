...
// Sparse overload
SolverResult gmres(const SparseMatrix &A, const Vector &b, Vector &x, real tol, idx max_iter,
                   idx restart) {
...
// matvec
SolverResult gmres(const Matrix &A, const Vector &b, Vector &x, real tol, idx max_iter, idx restart,
                   Backend backend) {
    if (A.rows() != A.cols()) {
        throw std::invalid_argument("GMRES requires a square matrix");
    }
