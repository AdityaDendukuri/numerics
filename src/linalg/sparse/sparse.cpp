...
SparseMatrix SparseMatrix::from_csc(idx n_rows,
                                    idx n_cols,
                                    const std::vector<real>& vals,
  ...
  if (col_ptrs.size() != n_cols + 1) {
    throw std::invalid_argument(
      "SparseMatrix::from_csc: col_ptrs must have length n_cols+1");
  }
  ...
  for (idx j = 0; j < n_cols; ++j) {
    if (col_ptrs[j] > col_ptrs[j + 1]) {
      throw std::invalid_argument(
        "SparseMatrix::from_csc: col_ptrs must be nondecreasing");
    }
  }
  ...
  if (nnz > vals.size() || nnz > row_indices.size()) {
    throw std::invalid_argument(
      "SparseMatrix::from_csc: payload shorter than col_ptrs.back()");
  }
