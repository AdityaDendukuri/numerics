...
///     v_{j+1} = w / beta_j
///
/// This builds the symmetric tridiagonal T_k = V_k^T * A * V_k with diagonal
/// alpha and off-diagonal beta.  The Ritz values are the eigenvalues of T_k and
/// the Ritz vectors are V_k * (eigenvectors of T_k).
///
/// T_k is diagonalised with eig_sym (Jacobi), which is O(k^3)  -- cheap for k
/// << n.
...
LanczosResult lanczos(MatVecFn matvec,
    ...
    for (idx j = 0; j < max_steps; ++j) {
        ...
        // Full reorthogonalisation (MGS against all previous columns of V)
        // Also returns beta_j = ||w||
    ...
    // Using all 'steps' Lanczos vectors (not just k) gives far better Ritz
    // approximations.
    idx    m = steps;
    Matrix T(m, m, 0.0);
    for (idx j = 0; j < m; ++j) {
    ...
    // Ritz vectors for the top-nret eigenvalues: V_m * eigvecs_of_T[:,
    // m-nret..m-1]
