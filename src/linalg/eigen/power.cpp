...
PowerResult inverse_iteration(const Matrix &A, real sigma, real tol, idx max_iter,
                              Backend backend) {
    constexpr real tiny = 1e-300;
    const idx n = A.rows();
    if (A.cols() != n) {
...
PowerResult rayleigh_iteration(const Matrix &A, const Vector &x0, real tol, idx max_iter,
                               Backend backend) {
    const idx n = A.rows();
    if (A.cols() != n) {
