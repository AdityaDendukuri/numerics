...
void validate_update(idx size,
                     idx updated_size,
                     const Vector& diagonal,
                     const Matrix& vectors,
                     const Matrix& weights,
                     const Vector& result,
                     idx fallback_block_size) {
...
template<typename Direct>
InverseDiagonalUpdatePath subtract_corrections(const Vector& diagonal,
...
[[nodiscard]] Vector general_corrections(const Matrix& left,
                                         const Matrix& reduced_solution) {
...
[[nodiscard]] bool solve_is_accurate(const Matrix& matrix,
...
InverseDiagonalUpdatePath symmetric_woodbury(idx size,
                                             idx updated_size,
                                             const Vector& diagonal,
                                             const Matrix& vectors,
                                             const Matrix& weights,
  ...
  return subtract_corrections(diagonal,
                              symmetric_corrections(inverse_vectors, reduced_factor),
                              result,
                              direct);
}

template<typename Solve, typename SolveTranspose, typename Direct>
InverseDiagonalUpdatePath general_woodbury(idx size,
                                           idx updated_size,
                                           const Vector& diagonal,
                                           const Matrix& vectors,
                                           const Matrix& weights,
  ...
  return subtract_corrections(diagonal,
                              general_corrections(left, reduced_solution),
                              result,
                              direct);
}
...
InverseDiagonalUpdatePath inverse_diagonal_after_update(
  ...
  if (factor.singular || updated_factor.singular) {
    throw std::invalid_argument(
      "inverse_diagonal_after_update: factors must be nonsingular");
  }
  return general_woodbury(
    factor.LU.rows(),
    updated_factor.LU.rows(),
    diagonal,
    update_vectors,
    update_weights,
    result,
    ...
    [&](const Matrix& rhs, Matrix& solution) {
      lu_solve_transpose(factor, rhs, solution);
    },
