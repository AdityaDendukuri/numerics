...
__global__ void k_matvec(const real* A,
                         const real* x,
                         real*       y,
                         idx         rows,
                         idx         cols) {
...
__global__ void k_matmul(const real* A,
                         const real* B,
                         real*       C,
                         idx         m,
                         idx         k,
...
__global__ void k_thomas_batched(const real* a,
                                 const real* b,
                                 const real* c,
                                 const real* d,
                                 real*       x,
...
void to_device(real* dst, const real* src, idx n) {
    cudaMemcpy(dst, src, n * sizeof(real), cudaMemcpyHostToDevice);
}
void to_host(real* dst, const real* src, idx n) {
    cudaMemcpy(dst, src, n * sizeof(real), cudaMemcpyDeviceToHost);
}
void scale(real* v, idx n, real alpha) {
    k_scale<<<(n + BLOCK - 1) / BLOCK, BLOCK>>>(v, n, alpha);
}
void add(const real* x, const real* y, real* z, idx n) {
    k_add<<<(n + BLOCK - 1) / BLOCK, BLOCK>>>(x, y, z, n);
}
void axpy(real alpha, const real* x, real* y, idx n) {
    k_axpy<<<(n + BLOCK - 1) / BLOCK, BLOCK>>>(alpha, x, y, n);
}
...
void thomas_batched(const real* a,
    ...
    k_thomas_batched<<<(batch_size + BLOCK - 1) / BLOCK, BLOCK>>>(a,
                                                                  b,
                                                                  c,
                                                                  d,
                                                                  x,
