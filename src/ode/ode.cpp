...
EulerSteps::EulerSteps(ODERhsFn f, Vector y0, ODEParams p)
    : f_(std::move(f)), y_(std::move(y0)), dydt_(y_.size()), t_(p.t0),
      t1_(p.tf), h_(p.h) {}
...
RK4Steps::RK4Steps(ODERhsFn f, Vector y0, ODEParams p)
    : f_(std::move(f)), y_(std::move(y0)), k1_(y_.size()), k2_(y_.size()),
      k3_(y_.size()), k4_(y_.size()), ytmp_(y_.size()), t_(p.t0), t1_(p.tf),
      h_(p.h) {}
...
RK45Steps::RK45Steps(ODERhsFn f, Vector y0, ODEParams p)
    : f_(std::move(f)), y_(std::move(y0)), k1_(y_.size()), k2_(y_.size()),
      k3_(y_.size()), k4_(y_.size()), k5_(y_.size()), k6_(y_.size()),
      k7_(y_.size()), ytmp_(y_.size()), err_(y_.size()), t_(p.t0), t1_(p.tf),
      h_(std::min(p.h, p.tf - p.t0)), rtol_(p.rtol), atol_(p.atol),
...
void RK45Steps::advance() {
  ...
  for (;;) {
    ...
    for (idx i = 0; i < n; ++i) {
      ytmp_[i] = y_[i] + (h_ * ((rk45_a41 * k1_[i]) + (rk45_a42 * k2_[i]) +
                                (rk45_a43 * k3_[i])));
    }
    ...
    for (idx i = 0; i < n; ++i) {
      ytmp_[i] = y_[i] + (h_ * ((rk45_a61 * k1_[i]) + (rk45_a62 * k2_[i]) +
                                rk45_a63 * k3_[i] + rk45_a64 * k4_[i] +
                                rk45_a65 * k5_[i]));
    }
    f_(t_ + h_, ytmp_, k6_);

    for (idx i = 0; i < n; ++i) {
      ytmp_[i] = y_[i] + (h_ * ((rk45_b1 * k1_[i]) + rk45_b3 * k3_[i] +
                                rk45_b4 * k4_[i] + (rk45_b5 * k5_[i]) +
                                (rk45_b6 * k6_[i])));
    }
    f_(t_ + h_, ytmp_, k7_);
    ...
    for (idx i = 0; i < n; ++i) {
      err_[i] =
          h_ * ((k1_[i] * rk45_e1) + (rk45_e3 * k3_[i]) + (rk45_e4 * k4_[i]) +
...
VerletSteps::VerletSteps(AccelFn accel, Vector q0, Vector v0, ODEParams p)
    : accel_(std::move(accel)), q_(std::move(q0)), v_(std::move(v0)),
      a_cur_(q_.size()), a_next_(q_.size()), t_(p.t0), t1_(p.tf), h_(p.h) {
...
Yoshida4Steps::Yoshida4Steps(AccelFn accel, Vector q0, Vector v0, ODEParams p)
    : accel_(std::move(accel)), q_(std::move(q0)), v_(std::move(v0)),
      acc_(q_.size()), t_(p.t0), t1_(p.tf), h_(p.h) {}
...
RK4_2ndSteps::RK4_2ndSteps(AccelFn accel, Vector q0, Vector v0, ODEParams p)
    : accel_(std::move(accel)), q_(std::move(q0)), v_(std::move(v0)),
      ...
      qtmp_(q_.size()), t_(p.t0), t1_(p.tf), h_(p.h) {}
...
// High-level wrappers — run to completion, call obs at each accepted step if
// provided.
...
SymplecticResult ode_verlet(AccelFn a, Vector q0, Vector v0, ODEParams p,
                            SympObserverFn obs) {
...
SymplecticResult ode_yoshida4(AccelFn a, Vector q0, Vector v0, ODEParams p,
                              SympObserverFn obs) {
...
SymplecticResult ode_rk4_2nd(AccelFn a, Vector q0, Vector v0, ODEParams p,
                             SympObserverFn obs) {
