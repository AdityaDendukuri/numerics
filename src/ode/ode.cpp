...
void RK45Steps::advance() {
    ...
    for (;;) {
        ...
        for (idx i = 0; i < n; ++i)
            err_[i] = h_
                      * (rk45_e1 * k1_[i] + rk45_e3 * k3_[i] + rk45_e4 * k4_[i]
                         + rk45_e5 * k5_[i] + rk45_e6 * k6_[i]
                         + rk45_e7 * k7_[i]);

        real err_norm = 0;
        for (idx i = 0; i < n; ++i) {
            real sc = atol_
                      + rtol_ * std::max(std::abs(y_[i]), std::abs(ytmp_[i]));
