...
void RK45Steps::advance() {
    ...
    for (;;) {
        ...
        for (idx i = 0; i < n; ++i) {
            ytmp_[i] =
                y_[i] + (h_ * ((rk45_a41 * k1_[i]) + (rk45_a42 * k2_[i]) + (rk45_a43 * k3_[i])));
        }
        ...
        for (idx i = 0; i < n; ++i) {
            ytmp_[i] =
                y_[i] + (h_ * ((rk45_a61 * k1_[i]) + (rk45_a62 * k2_[i]) + (rk45_a63 * k3_[i]) +
        ...
        for (idx i = 0; i < n; ++i) {
            err_[i] = h_ * ((k1_[i] * rk45_e1) + (rk45_e3 * k3_[i]) + (rk45_e4 * k4_[i]) +
...
ODEResult ode_euler(ODERhsFn f, Vector y0, ODEParams p, const ObserverFn &obs) {
...
ODEResult ode_rk4(ODERhsFn f, Vector y0, ODEParams p, const ObserverFn &obs) {
...
ODEResult ode_rk45(ODERhsFn f, Vector y0, ODEParams p, const ObserverFn &obs) {
