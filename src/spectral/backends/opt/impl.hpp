...
static inline void butterfly(num::cplx* __restrict__ u,
    ...
    for (; j + 1 < hlen; j += 2) {
        ...
        __m256d T   = _mm256_addsub_pd(_mm256_mul_pd(V, Wre),
...
static inline void butterfly(num::cplx* __restrict__ u,
    ...
    for (; j + 1 < hlen; j += 2) {
        ...
        // Tr = Vr*Wr - Vi*Wi
        float64x2_t Tr =
            vfmsq_f64(vmulq_f64(V.val[0], W.val[0]), V.val[1], W.val[1]);
        // Ti = Vr*Wi + Vi*Wr
        float64x2_t Ti =
            vfmaq_f64(vmulq_f64(V.val[0], W.val[1]), V.val[1], W.val[0]);
...
static inline void butterfly(num::cplx*       u,
                             num::cplx*       v,
                             const num::cplx* tw,
                             int              hlen) {
