...
///
/// Applies \f$(u,v)\leftarrow(u+vw,u-vw)\f$ with AVX2, NEON, or scalar code.
#pragma once
...
static inline void butterfly(num::cplx *__restrict__ u, num::cplx *__restrict__ v,
                             const num::cplx *__restrict__ tw, int hlen) {
    ...
    for (; j + 1 < hlen; j += 2) {
        ...
        __m256d T = _mm256_addsub_pd(_mm256_mul_pd(V, Wre), _mm256_mul_pd(Vsw, Wim));
...
static inline void butterfly(num::cplx *__restrict__ u, num::cplx *__restrict__ v,
    ...
    for (; j + 1 < hlen; j += 2) {
        ...
        float64x2x2_t U = vld2q_f64(ud + static_cast<ptrdiff_t>(2 * j));
        float64x2x2_t V = vld2q_f64(vd + static_cast<ptrdiff_t>(2 * j));
        float64x2x2_t W = vld2q_f64(wd + static_cast<ptrdiff_t>(2 * j));

        // T = V * W (complex multiply, component-wise on SoA data).
        float64x2_t tr = vfmsq_f64(vmulq_f64(V.val[0], W.val[0]), V.val[1], W.val[1]);
        // Ti = Vr*Wi + Vi*Wr.
        float64x2_t ti = vfmaq_f64(vmulq_f64(V.val[0], W.val[1]), V.val[1], W.val[0]);
...
static inline void butterfly(num::cplx *u, num::cplx *v, const num::cplx *tw, int hlen) {
...
struct FFTPlanImpl : public num::spectral::FFTPlanImpl {
    ...
    void execute_inplace(num::CVector &out) const {
