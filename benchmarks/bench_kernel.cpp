...
///
/// Three groups:
///   Tier 2 array/reduce  -- dispatch::array::axpby and dispatch::reduce::l1_norm
...
using namespace num;
namespace ks = num::dispatch::subspace;
namespace ka = num::seq;
namespace kao = num::omp;
namespace kr = num::seq;
namespace kro = num::omp;
...
static void BM_Raw_AxpyNorm_Separate(benchmark::State &state) {
    ...
    for (auto _ : state) {
        ...
        real norm_sq = kernel::norm_sq(y.data(), n);
...
// Basis stored as columns of a row-major mat.
...
static void BM_MathSpine_CG_Generic(benchmark::State &state) {
    ...
    for (auto _ : state) {
        ...
        auto result = cg(A, b, x, cg_options{.tolerance = 1e-10, .max_iterations = 100});
        benchmark::DoNotOptimize(result);
