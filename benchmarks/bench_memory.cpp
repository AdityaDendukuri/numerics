...
void* operator new(std::size_t size) {
  ...
  if (mem::detail::g_active.load(std::memory_order_relaxed)) {
    ...
    while (live > peak
           && !g_peak.compare_exchange_weak(peak,
                                            live,
...
void operator delete(void* p, std::size_t size) noexcept {
  if (mem::detail::g_active.load(std::memory_order_relaxed))
    mem::detail::g_live.fetch_sub(static_cast<int64_t>(size), std::memory_order_relaxed);
  std::free(p);
}
...
void mem::print_report(const std::vector<benchmark::BenchmarkReporter::Run>& results,
                       std::ostream& out) {
  ...
  for (auto* r : mem_runs) {
    ...
    // Rather than recompute, we show total_allocated_bytes / num_allocs *
    // allocs_per_iter which simplifies to: total_allocated_bytes /
    // memory_iterations. Just show the raw totals alongside per-iter allocs
    // for clarity.
    int64_t total =
      (mr.total_allocated_bytes == tombstone) ? -1 : mr.total_allocated_bytes;
    ...
    // memory_iterations = num_allocs / allocs_per_iter  (if allocs_per_iter
    // > 0).
    ...
    out << std::left << std::setw(static_cast<int>(name_w)) << r->run_name.str() << "  "
        ...
        << "  " << std::setw(col) << bytes_col << "  " << std::setw(col)
        << fmt_bytes(peak) << "  " << std::setw(col) << fmt_bytes(net) << note << "\n";
  }
  out << sep << "\n\n";
  out << "Notes:\n"
      << "  allocs/iter  : heap allocations per benchmark iteration (setup + "
         "hot path)\n"
      << "  bytes/iter   : total bytes requested from allocator per "
         "iteration\n"
      << "  peak live    : maximum simultaneously live heap during the run\n"
      << "  net growth   : bytes still allocated after the run (tracked via "
         "sized delete)\n"
      << "  [alloc-free] : hot path never touched the heap — ideal for "
         "production kernels\n\n";
}
