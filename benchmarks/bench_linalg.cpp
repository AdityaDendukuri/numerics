...
static void BM_Matmul_Naive(benchmark::State& state) {
  ...
  state.counters["GFLOP/s"] =
    benchmark::Counter(matmul_flops(n),
...
BENCHMARK_TEMPLATE(BM_Matmul, Backend::blocked)
  ->RangeMultiplier(2)
  ->Range(64, 512)
  ->Complexity();
BENCHMARK_TEMPLATE(BM_Matmul, Backend::simd)
  ->RangeMultiplier(2)
  ->Range(64, 512)
  ->Complexity();
BENCHMARK_TEMPLATE(BM_Matmul, Backend::blas)
  ->RangeMultiplier(2)
  ->Range(64, 512)
  ->Complexity();
BENCHMARK_TEMPLATE(BM_Matmul, Backend::omp)
  ->RangeMultiplier(2)
  ->Range(64, 512)
  ->Complexity();
...
static void BM_Thomas(benchmark::State& state) {
  ...
  state.SetBytesProcessed(state.iterations() * static_cast<int64_t>(5 * n - 2)
                          * sizeof(real));
  state.SetComplexityN(static_cast<int64_t>(n));
}
