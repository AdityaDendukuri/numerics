...
/// Prefers complexity_n (set by SetComplexityN); falls back to parsing /N from
/// the name.
static double size_of(const Run& r) {
  ...
  if (pos != std::string::npos)
    try {
      return std::stod(name.substr(pos + 1));
    } catch (...) {}
  return 0.0;
}
...
static Series series_by_counter(const std::vector<Run>& runs,
  ...
  for (auto& r : runs) {
    if (r.benchmark_name().find(name_substr) == std::string::npos)
      continue;
    if (r.benchmark_name().find("BigO") != std::string::npos)
      continue;
    if (r.benchmark_name().find("RMS") != std::string::npos)
      continue;
    double v = counter(r, counter_key);
    if (v > 0)
      s.emplace_back(size_of(r), v);
  }
...
static std::string plot_cmd(const std::vector<std::pair<std::string, int>>& series) {
  ...
  for (size_t i = 0; i < series.size(); ++i) {
    if (i)
      cmd += ", ";
    cmd += "'-' with linespoints ls " + std::to_string(series[i].second) + " title '"
           + series[i].first + "'";
  }
  cmd += "\n";
  return cmd;
...
static void plot_matmul(Gnuplot& gp,
  ...
  for (auto& v : variants) {
    ...
    if (!s.empty())
      data.emplace_back(v, std::move(s));
  }
  if (data.empty())
    return;
  ...
  for (auto& [v, _] : data)
    labels.emplace_back(v.label, v.ls);
  gp << plot_cmd(labels);
  for (auto& [_, s] : data)
    gp.send1d(s);
}
...
static void plot_matvec(Gnuplot& gp,
                        const std::vector<Run>& runs,
                        const std::string& outdir,
                        const std::string& ext = ".pdf") {
  struct Variant {
  ...
  for (auto& v : variants) {
    ...
    for (auto& r : runs) {
      ...
      if (bps > 0)
        s.emplace_back(size_of(r), bps / 1e9);
    }
    std::sort(s.begin(), s.end());
    if (!s.empty())
      data.emplace_back(v, std::move(s));
...
static void plot_dot_axpy(Gnuplot& gp,
                          const std::vector<Run>& runs,
                          const std::string& outdir,
                          const std::string& ext = ".pdf") {
  struct Op {
...
static void plot_cg(Gnuplot& gp,
                    const std::vector<Run>& runs,
                    const std::string& outdir,
                    const std::string& ext = ".pdf") {
  ...
  gp << "set output '" + outdir + "/cg" + ext + "'\n"
     << "set title 'Conjugate Gradient: time vs system size' font "
        "'Times-Bold,14'\n"
     << "set xlabel 'n' font 'Times,12'\n"
...
static void plot_thomas(Gnuplot& gp,
                        const std::vector<Run>& runs,
                        const std::string& outdir,
                        const std::string& ext = ".pdf") {
...
static void plot_fft(Gnuplot& gp,
                     const std::vector<Run>& runs,
                     const std::string& outdir,
                     const std::string& ext = ".pdf") {
  struct Variant {
  ...
  // -- forward FFT: one-shot -----------------------------------------------
  {
    ...
    if (!data.empty()) {
      gp << "set output '" + outdir + "/fft" + ext + "'\n"
         << "set title 'FFT (forward): memory throughput' font "
            "'Times-Bold,14'\n"
         << "set xlabel 'n' font 'Times,12'\n"
         << "set ylabel 'GB/s' font 'Times,12'\n";
      set_loglog(gp);
  ...
  // -- reusable plan: seq vs fftw ------------------------------------------
  {
    ...
    if (!data.empty()) {
      gp << "set output '" + outdir + "/fft_plan" + ext + "'\n"
         << "set title 'FFT Plan (reusable): memory throughput' font "
            "'Times-Bold,14'\n"
         << "set xlabel 'n' font 'Times,12'\n"
         << "set ylabel 'GB/s' font 'Times,12'\n";
      set_loglog(gp);
  ...
  // -- rfft: seq vs fftw ---------------------------------------------------
  {
    ...
    if (!data.empty()) {
      gp << "set output '" + outdir + "/rfft" + ext + "'\n"
         << "set title 'rFFT (real-to-complex): memory throughput' font "
            "'Times-Bold,14'\n"
         << "set xlabel 'n' font 'Times,12'\n"
         << "set ylabel 'GB/s' font 'Times,12'\n";
      set_loglog(gp);
...
static void plot_banded(Gnuplot& gp,
                        const std::vector<Run>& runs,
                        const std::string& outdir,
                        const std::string& ext = ".pdf") {
  struct Variant {
...
static void plot_lu(Gnuplot& gp,
                    const std::vector<Run>& runs,
                    const std::string& outdir,
                    const std::string& ext = ".pdf") {
  struct Variant {
...
static void plot_qr(Gnuplot& gp,
                    const std::vector<Run>& runs,
                    const std::string& outdir,
                    const std::string& ext = ".pdf") {
  struct Variant {
...
static void plot_svd(Gnuplot& gp,
                     const std::vector<Run>& runs,
                     const std::string& outdir,
                     const std::string& ext = ".pdf") {
  struct Variant {
  ...
  for (auto& v : variants) {
    auto s = series_by_counter(runs, v.key, "GFLOP/s");
    if (s.empty())
      s = series_by_time(runs,
                         v.key); // fallback to time if no GFLOP/s counter
    if (!s.empty())
      data.emplace_back(v, std::move(s));
  }
  if (data.empty())
...
static void plot_eigen(Gnuplot& gp,
                       const std::vector<Run>& runs,
                       const std::string& outdir,
                       const std::string& ext = ".pdf") {
  struct Variant {
  ...
  gp << "set output '" + outdir + "/eigen" + ext + "'\n"
     << "set title 'Symmetric Eigensolver: time ({/Symbol m}s) vs n' font "
        "'Times-Bold,14'\n"
     << "set xlabel 'n' font 'Times,12'\n"
     << "set ylabel 'Time ({/Symbol m}s)' font 'Times,12'\n";
  set_loglog(gp);
...
public:
  GnuplotAscii& operator<<(const std::string& cmd) {
    script_ += cmd;
    return *this;
  }
  ...
  void write(const std::string& outpath, int w = 78, int h = 18) const {
    ...
    if (!pipe)
      return;
    std::string hdr = "set terminal dumb " + std::to_string(w) + " " + std::to_string(h)
                      + " noenhanced\n" + "set output '" + outpath + "'\n";
...
static void ascii_throughput(
  ...
  for (auto& [key, label] : variants) {
    ...
    std::sort(s.begin(), s.end());
    if (!s.empty())
      data.emplace_back(label, std::move(s));
  }
  if (data.empty())
    return;
...
static void ascii_time(const std::vector<Run>& runs,
                       const std::string& outpath,
                       const std::string& title,
...
inline void plot_all_ascii(const std::vector<Run>& runs,
  ...
  ascii_throughput(runs,
                   outdir + "/matvec.txt",
                   "Matrix-Vector Multiply: GB/s vs n",
                   "GB/s",
                   {
  ...
  ascii_time(runs,
             outdir + "/cg.txt",
             "Conjugate Gradient: time vs n",
             {{"BM_CG/", "CG"}});
  ascii_time(runs,
             outdir + "/thomas.txt",
             "Thomas Algorithm: time vs n",
             {{"BM_Thomas/", "Thomas"}});
  ascii_time(runs,
             outdir + "/banded.txt",
             "Banded Solver: time vs n",
             {{"BM_Band", "banded"}});

  // FFT: forward, plan, rfft -- one file each
  for (auto& [bm, file, title] :
