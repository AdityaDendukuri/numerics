...
## 2D Poisson solver (`pde/poisson.hpp`) {#sec_pde_poisson}

Solves \f$-\nabla^2 u = f\f$ on \f$(0,1)^2\f$ with homogeneous Dirichlet BCs via the
Discrete Sine Transform.  Two variants:

- `pde::poisson2d_fd` -- FD eigenvalues \f$D_k = 2(1-\cos(k\pi/(N+1)))\f$, error \f$O(h^2)\f$.
- `pde::poisson2d` -- exact eigenvalues \f$(k\pi)^2\f$, machine-precision error for \f$f\f$
  in the DST eigenbasis.

Both run in \f$O(N^2 \log N)\f$ and require \f$N = 2^p - 1\f$.
See [poisson.md](poisson.md) for the full derivation and convergence table.
