...
Callables conforming to `num::scalar_fn` (e.g. `double(double)` lambdas):
...
double trapz(num::scalar_fn f, double a, double b, num::idx n);
```
...
```cpp
template <bool Parallel = false>
double simpson(num::scalar_fn f, double a, double b, num::idx n);
```

```cpp
double q = num::simpson(gaussian, 0.0, 1.0, 4096);
double q_omp = num::simpson<true>(gaussian, 0.0, 1.0, 1 << 20); // OpenMP
```
...
```cpp
double gauss_legendre(num::scalar_fn f, double a, double b, int n = 5);
```
...
```cpp
double adaptive_simpson(num::scalar_fn f, double a, double b, double tol = 1e-10);
```
...
```cpp
double romberg(num::scalar_fn f, double a, double b, double tol = 1e-12, int max_depth = 12);
```
...
## 5. Inverse Laplace Contour Integration (num::talbot_quadrature)
