# Core Objects and Helpers {#page_core}
...
num::matmul(A, A, C, num::best_backend);  // Best compiled backend.
```
...
```cpp
bool symmetric = num::linalg::is_symmetric(A_dense);
bool positive_definite = num::linalg::is_spd(A_dense);
```

### Validate and wrap a property

```cpp
auto symmetric = num::linalg::make_symmetric(A_dense); // Checks A==A^T.
auto spd = num::linalg::make_spd(A_dense);             // Checks A==A^T>0.
```

```cpp
auto spd = num::linalg::make_spd(indefinite); // Throws when validation fails.
```

### Declare a construction-guaranteed property

```cpp
auto symmetric = num::linalg::assume_symmetric(A_dense);
auto spd = num::linalg::assume_spd(A_dense);
