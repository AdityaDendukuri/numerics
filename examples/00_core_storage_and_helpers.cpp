...
int main() {
  ...
  const real span_dot = dot(std::span<const real>(x.data(), x.size()),
                            std::span<const real>(y.data(), y.size()));
  ...
  const SparseMatrix sparse =
    SparseMatrix::from_triplets(3,
                                3,
                                std::vector<idx>{0, 0, 1, 2},
  ...
  const auto smallest =
    smallest_indices(std::span<const real>(diag.data(), diag.size()), 2);
  Vector probability{0.2, -0.1, 0.8};
  const real clipped_mass = clip_and_normalize_nonnegative(
    std::span<real>(probability.data(), probability.size()));
  ...
  std::cout << xy + span_dot + length + points.x(1) + Ax[0] + product(0, 0) + sum(0, 0)
                 + e1[1] + I(0, 0) + rhs(1, 0) + D(0, 0) + scattered[2] + sparse_x[0]
                 + sparse_t(0, 0) + half(0, 0) + sparse_dense(0, 0) + sparse_diag[0]
                 + similar(0, 0) + static_cast<real>(symmetric)
                 + static_cast<real>(positive_definite) + checked_spd.base()(0, 0)
                 + assumed_spd.base()(0, 0)
                 + static_cast<real>(largest + smallest.front()) + clipped_mass
                 + expectation
            << '\n';
}
