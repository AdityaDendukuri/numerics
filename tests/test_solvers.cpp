...
TEST(CG, DiagonalDominant5x5) {
    ...
    matvec(A, x, Ax);
    real err = 0;
    for (idx i = 0; i < n; ++i) err += (Ax[i]-b[i])*(Ax[i]-b[i]);
...
TEST(CG, ConvergesWithinN) {
    idx n = 10;
    Matrix A(n, n, 0.0);
    for (idx i = 0; i < n; ++i) A(i,i) = static_cast<real>(i+1);

    Vector b(n), x(n, 0.0);
    for (idx i = 0; i < n; ++i) b[i] = static_cast<real>(i+1);

    SolverResult r = cg(A, b, x);
    EXPECT_TRUE(r.converged);
    EXPECT_LE(r.iterations, n);
    for (idx i = 0; i < n; ++i) EXPECT_NEAR(x[i], 1.0, 1e-9);
}
...
TEST(Thomas, Small4x4) {
    ...
    thomas(a, b, c, d, x);
    for (idx i = 0; i < 4; ++i) EXPECT_NEAR(x[i], 1.0, 1e-10);
}
...
TEST(GaussSeidel, DiagonalSystem) {
    ...
    Vector b(n), x(n, 0.0);
    for (idx i = 0; i < n; ++i) b[i] = static_cast<real>((i+1)*(i+1));

    SolverResult r = gauss_seidel(A, b, x);
    EXPECT_TRUE(r.converged);
    for (idx i = 0; i < n; ++i) EXPECT_NEAR(x[i], static_cast<real>(i+1), 1e-8);
}
...
TEST(GaussSeidel, ResidualVerified) {
    ...
    Vector Ax(n);
    matvec(A, x, Ax);
    for (idx i = 0; i < n; ++i) EXPECT_NEAR(Ax[i], b[i], 1e-8);
}
...
TEST(Jacobi, DiagonalSystem) {
    ...
    for (idx i = 0; i < n; ++i) EXPECT_NEAR(x[i], static_cast<real>(i+1), 1e-10);
}
...
TEST(GMRES, SparseLaplacian1D) {
    ...
    for (idx i = 0; i < n; ++i) EXPECT_NEAR(Ax[i], b[i], 1e-5);
}
...
TEST(GMRES, MatrixFree) {
    ...
    idx n = 5;
    Vector diag(n);
    for (idx i = 0; i < n; ++i) diag[i] = static_cast<real>(i + 1);

    MatVecFn mv = [&](const Vector& in, Vector& out) {
        out = Vector(n);
        for (idx i = 0; i < n; ++i) out[i] = diag[i] * in[i];
    };
...
TEST(SparseMatrix, FromTriplets) {
    // 3x3 identity
    SparseMatrix I = SparseMatrix::from_triplets(3, 3,
...
TEST(SparseMatrix, DuplicatesSummed) {
    ...
    SparseMatrix A = SparseMatrix::from_triplets(2, 2,
        {0,0,1}, {0,0,1}, {1.0,2.0,4.0});
...
TEST(SparseMatrix, Matvec) {
    ...
    SparseMatrix A = SparseMatrix::from_triplets(2, 2,
        {0,0,1,1}, {0,1,0,1}, {2.0,-1.0,-1.0,2.0});
