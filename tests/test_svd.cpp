...
static Matrix make_lowrank(idx n, idx k) {
    // U: random-ish orthogonal factor (use Hilbert columns, then orthonormalize
    // via QR) For simplicity use the first k standard basis vectors scaled by
    // s_i. A = sum_i s_i * e_i * e_i^T  (diagonal, trivially rank-k)
    Matrix A(n, n, 0.0);
    for (idx i = 0; i < k; ++i)
