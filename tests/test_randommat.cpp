...
TEST(RandomMat, ApproxCholBasicGraphFactorizeSolve) {
    ...
    SparseMatrix L = num::linear::laplacian(G);
    Vector Lx(4, 0.0);
...
TEST(RandomMat, ApproxCholPreconditionerWithPCG) {
    ...
    const auto sampled_prec_on_zero_sum =
        num::assume<axiom::positive_definite_on<space::zero_sum>>(projected_sampled_prec);
    Vector x_sampled(n, 0.0);
